...
void InitializeMFSurfchemNamespace()
{
    ...
    stoch_surfcov0 = 0; // default value
    pp.query("stoch_surfcov0",stoch_surfcov0);

    stoch_MFsurfchem = 0; // default value
    ...
    use_ml_mfsurfchem = 0; // default: standard method
    pp.query("use_ml_mfsurfchem", use_ml_mfsurfchem);

    if (use_ml_mfsurfchem == 1) {
        ...
        try {
            ...
            Abort("Error loading MFsurfchem ML model");
        }
    }
    return;
}
                       ...
                       const amrex::Geometry& geom, const amrex::Real dt)
{
    ...
    for (MFIter mfi(cu,false); mfi.isValid(); ++mfi)
    {
        ...
        if (use_ml_mfsurfchem == 1) {
            ...
            std::vector<amrex::Real> ml_inputs;
            std::vector<int> cell_i;
            std::vector<int> cell_j;
            std::vector<int> cell_k;
            std::vector<int> cell_m;

            ml_inputs.reserve(bx.numPts() * n_ads_spec * 3);
            cell_i.reserve(bx.numPts() * n_ads_spec);
            cell_j.reserve(bx.numPts() * n_ads_spec);
    #if AMREX_SPACEDIM == 3
            cell_k.reserve(bx.numPts() * n_ads_spec);
    #endif
            cell_m.reserve(bx.numPts() * n_ads_spec);
                    ...
                    for (int i = bx.smallEnd(0); i <= bx.bigEnd(0); ++i) {
                        ...
                        if (on_wall) {
                            ...
                            for (int m = 0; m < n_ads_spec; ++m) {
                                ...
                                ml_inputs.push_back(pres);
                                ml_inputs.push_back(tempratio);
                                ml_inputs.push_back(theta);
            ...
            if (!ml_inputs.empty()) {
                at::Tensor inputs_torch = torch::from_blob(
                    ml_inputs.data(),
                    {static_cast<long>(cell_i.size()), 3},
                ...
                auto outputs_acc = outputs_torch.accessor<amrex::Real,2>();

                for (std::size_t s = 0; s < cell_i.size(); ++s) {
                    int i = cell_i[s];
                    int j = cell_j[s];
    #if AMREX_SPACEDIM == 3
                    int k = cell_k[s];
                    ...
                    amrex::Real pred_ads = outputs_acc[s][0];
                    amrex::Real pred_des = outputs_acc[s][1];

                    pred_ads = std::clamp(pred_ads, amrex::Real(0.0), amrex::Real(1.0));
                    pred_des = std::clamp(pred_des, amrex::Real(0.0), amrex::Real(1.0));

                    amrex::Real meanNads;
                    amrex::Real meanNdes;

                    meanNads = static_cast<amrex::Real>(std::lround(pred_ads * Ntot));
                    meanNdes = static_cast<amrex::Real>(std::lround(pred_des * Ntot));
                    ...
                    if (stoch_MFsurfchem==0) {
                        ...
                        Nads = amrex::RandomPoisson(meanNads);
                        Ndes = amrex::RandomPoisson(meanNdes);
                    }

                    //amrex::Print() << "DATA " << ml_inputs[3*s] << " " << ml_inputs[3*s + 1] << " "
                                   //<< ml_inputs[3*s + 2] << " " << Nads / Ntot << " " << Ndes / Ntot << "\n";
                ...
                if ( (ads_wall_dir == 0 && i == 0) || (ads_wall_dir == 1 && j == 0) || (ads_wall_dir == 2 && k == 0) ) {
                    ...
                    for (int m=0;m<n_ads_spec;m++) {
                        ...
                        if (stoch_MFsurfchem==0) {
                            ...
                            Nads = amrex::RandomPoisson(meanNads,engine);
                            Ndes = amrex::RandomPoisson(meanNdes,engine);
                        }

                        //amrex::Print() << "DATA " << pres << " " << tempratio << " " << theta << " " << Nads/Ntot << " " << Ndes/Ntot << "\n";
