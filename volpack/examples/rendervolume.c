...
void StorePGM(unsigned char* image, int width, int height, char* filename);
...
char **argv;
{
	...
	if ((clvolume_fd = open(CLVOLUME_FILE, O_BINARY)) < 0) {
	    perror("open");
    ...
    vpSetImage(vpc, (unsigned char *)image[0], IMAGE_WIDTH, IMAGE_HEIGHT,
	...
	/* store image */
	if (numframes == 1)
	    strcpy(filename, "brainsmall.pgm");
	else
	    sprintf(filename, "brainsmall_%d.pgm", n + 1000);
